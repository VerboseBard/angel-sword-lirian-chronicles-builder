...
(function () {
  ...
  const ROLLER_VERSION = "dice-lab-scripted-side-entry-26-rana-d4-measured-medallion";
  ...
  /* Rana d4 corner repair, third design (owner review 2026-08-25): the
     painted medallion circles are correctly placed and sized — they are
     KEPT untouched. Only the misplaced NUMBER strokes are removed and
     redrawn centered in the true circle. Each circle is measured from the
     art by circumference scoring (a straight frame rail can only match a
     short arc of any candidate circle, so unlike centroid detection it
     cannot fake a ring), the cream number strokes are erased — disc color
     inside the ring, local background blend outside it — and the number is
     redrawn centered at the painted circle's own scale. Faces where a ring
     cannot be confidently measured are left untouched. */
  const RANA_D4_CORNER_LANDMARKS = [
    { key: "apex", x: 0.5, y: 0.2, rotation: 0 },
    { key: "right", x: 0.8, y: 0.85, rotation: (Math.PI * 2) / 3 },
    { key: "left", x: 0.2, y: 0.85, rotation: -(Math.PI * 2) / 3 }
  ];

  function houghDetectMedallion(data, size, nominalX, nominalY) {
    const window = size * 0.1;
    const goldness = (px, py) => {
      const x = px | 0;
      const y = py | 0;
      ...
      return r > 130 && g > 85 && b < 135 && r > g && g > b ? 1 : 0;
    };
    const angles = [];
    for (let a = 0; a < 40; a += 1) {
      angles.push((Math.PI * 2 * a) / 40);
    }
    const scoreAt = (cx, cy, radius) => {
      let hits = 0;
      for (const angle of angles) {
        hits += goldness(cx + radius * Math.cos(angle), cy + radius * Math.sin(angle));
      }
      return hits / angles.length;
    };
    let best = { score: 0, cx: nominalX, cy: nominalY, radius: size * 0.05 };
    const rMin = Math.round(size * 0.036);
    const rMax = Math.round(size * 0.07);
    for (let cy = nominalY - window; cy <= nominalY + window; cy += 2) {
      for (let cx = nominalX - window; cx <= nominalX + window; cx += 2) {
        for (let radius = rMin; radius <= rMax; radius += 2) {
          const score = scoreAt(cx, cy, radius);
          if (score > best.score) {
    ...
    for (let cy = best.cy - 1; cy <= best.cy + 1; cy += 1) {
      for (let cx = best.cx - 1; cx <= best.cx + 1; cx += 1) {
        for (let radius = best.radius - 1; radius <= best.radius + 1; radius += 1) {
          const score = scoreAt(cx, cy, radius);
          if (score > best.score) {
            best = { score, cx, cy, radius };
          }
  ...
  function drawRanaD4CornerNumbers(context, artKey, size) {
    ...
    const isNumeralPixel = (x, y) => {
      const offset = (y * size + x) * 4;
      const r = data[offset];
      const g = data[offset + 1];
      const b = data[offset + 2];
      // Cream numeral: bright with a HIGH blue channel; the gold ring and
      // sparkle glints have low blue and survive this filter.
      return r + g + b > 430 && b > 135;
    };
    const detected = RANA_D4_CORNER_LANDMARKS
      .map((landmark) => ({ landmark, ring: houghDetectMedallion(data, size, landmark.x * size, landmark.y * size) }))
      .filter((entry) => entry.ring);
    // Pass 1: erase every corner's old number strokes in the pixel buffer.
    detected.forEach(({ ring }) => {
      const discRadius = ring.radius * 0.8;
      // Disc color: median of interior non-bright pixels.
      const darks = [];
      for (let y = Math.round(ring.cy - discRadius); y <= ring.cy + discRadius; y += 2) {
        for (let x = Math.round(ring.cx - discRadius); x <= ring.cx + discRadius; x += 2) {
          if (Math.hypot(x - ring.cx, y - ring.cy) > discRadius) {
            continue;
          }
          const offset = (y * size + x) * 4;
          const r = data[offset];
      ...
      const channel = (index) => {
        ...
        return sorted.length ? sorted[Math.floor(sorted.length / 2)] : 16;
      };
      ring.discColor = [channel(0), channel(1), channel(2)];
      /* Outside the ring, erase the old number's OVERHANG. Rana numerals
         are cream fill wrapped in a dark outline the color filter cannot
         see, so the cream mask is DILATED a few pixels to swallow outline
         and shadow, then each masked pixel takes the median of unmasked
         neighbors. Inside the ring nothing is erased here — the opaque
         replacement disc covers every interior remnant. */
      const reach = Math.round(ring.radius * 1.8);
      const x0 = Math.max(0, Math.round(ring.cx - reach));
      const y0 = Math.max(0, Math.round(ring.cy - reach));
      const x1 = Math.min(size - 1, Math.round(ring.cx + reach));
      const y1 = Math.min(size - 1, Math.round(ring.cy + reach));
      const w = x1 - x0 + 1;
      const h = y1 - y0 + 1;
      const mask = new Uint8Array(w * h);
      for (let y = y0; y <= y1; y += 1) {
        for (let x = x0; x <= x1; x += 1) {
          if (isNumeralPixel(x, y)) {
            mask[(y - y0) * w + (x - x0)] = 1;
          }
        }
      }
      ...
      for (let y = y0; y <= y1; y += 1) {
        for (let x = x0; x <= x1; x += 1) {
          if (!dilated[(y - y0) * w + (x - x0)]) {
            continue;
          }
          const distance = Math.hypot(x - ring.cx, y - ring.cy);
          if (distance <= ring.radius * 1.02 || distance > reach) {
            continue;
          }
          const locals = [];
          for (let dy = -9; dy <= 9; dy += 3) {
            for (let dx = -9; dx <= 9; dx += 3) {
              const sx = x + dx;
              const sy = y + dy;
              if (sx < x0 || sy < y0 || sx > x1 || sy > y1 || dilated[(sy - y0) * w + (sx - x0)]) {
                continue;
              }
              const so = (sy * size + sx) * 4;
              locals.push([data[so], data[so + 1], data[so + 2]]);
            }
          }
          if (locals.length) {
            const pick = locals[Math.floor(locals.length / 2)];
            const offset = (y * size + x) * 4;
            data[offset] = pick[0];
            data[offset + 1] = pick[1];
            data[offset + 2] = pick[2];
          }
        }
      }
    });
    ...
    /* Pass 2: replacement medallion at the MEASURED position and the
       MEASURED size (owner requirement: same size as the painted circle),
       then the number dead-center. The opaque disc buries every interior
       remnant the color filter could not identify. */
    detected.forEach(({ landmark, ring }) => {
      const label = landmark.key === "apex" ? corners.apex : landmark.key === "right" ? corners.right : corners.left;
      const radius = ring.radius;
      const disc = `rgb(${ring.discColor[0]}, ${ring.discColor[1]}, ${ring.discColor[2]})`;
      context.save();
      context.beginPath();
      context.arc(ring.cx, ring.cy, radius * 0.97, 0, Math.PI * 2);
      context.fillStyle = disc;
      context.fill();
      const ringStroke = (width, color) => {
        context.beginPath();
        context.arc(ring.cx, ring.cy, radius * 0.97, 0, Math.PI * 2);
      ...
      ringStroke(radius * 0.17, "rgba(82, 43, 5, 0.98)");
      ringStroke(radius * 0.1, "rgba(211, 143, 35, 1)");
      ringStroke(radius * 0.04, "rgba(255, 223, 122, 0.95)");
      context.translate(ring.cx, ring.cy);
      context.rotate(landmark.rotation);
      context.textAlign = "center";
      context.textBaseline = "middle";
      context.font = `700 ${Math.round(radius * 1.12)}px Georgia, "Times New Roman", serif`;
      context.lineJoin = "round";
